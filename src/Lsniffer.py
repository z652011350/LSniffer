...
# 处理抓到的包并显示
def process_packet(p: packet):
    global pause_flag, stop_flag
    if pause_flag is False and stop_flag is False:
        global packet_list
        sport = 'null'
        dport = 'null'
        id = -1
        ...
        if Ether in p:
            src_ip = p[Ether].src
            dst_ip = p[Ether].dst
            proto_type = p[Ether].type
            proto_types = {0x0800: 'IPv4', 0x0806: 'ARP', 0x86dd: 'IPv6', 0x88cc: 'LLDP', 0x891D: 'TTE'}
            if proto_type in proto_types:
                proto = proto_types[proto_type]
            else:
                proto = 'LOOP'  # 协议
            # IP
            if proto == 'IPv4':
        ...
        if id_flag is True:
            return
        # id_flag = False
        # print('processed_packet id_flag = true')

        # print('processed_packet')
        global track_tcp_ip_port_flag, track_tcp_pid_flag

        # return
        processed_packet = [packet_id, packet_time, src_ip, dst_ip, proto, sport, dport, length, id, info]
        if track_tcp_ip_port_flag:
            ...
            # return 
            if not check_is_save(processed_packet, ip=[track_src_ip, track_dst_ip],
                                 port=[track_src_port, track_dst_port],
                                 flag=0, caped_packet=p):
                # id_flag = False
            ...
            global track_pid
            if not check_is_save(processed_packet, pid=track_pid,
                                 flag=0, caped_packet=p):
                # id_flag = False
                return
        # print('processed_packet')
        id_flag = True
        packet_list.append({'packet': p, 'processed_packet': processed_packet})
        ...
        # print('processed_packet id_flag = false')
        id_flag = False
    elif stop_flag is True:
...
def show_detail_data(event):
    ...
    if selected_item == ():
        ...
        # 等待packlist的数据
        while packet_list == [] and stop_flag is False:
            pass
        if stop_flag is True:
            return
        caped_packet = packet_list[packet_id - 1]['packet']
    else:
        ...
        # print(stop_flag,pause_flag,track_tcp_pid_flag,track_tcp_ip_port_flag)
        caped_packet = packet_list[local_id]['packet']
    # ## print(packet_list[packet_id])

    lines = (caped_packet.show(dump=True)).split('\n')
    ...
    raw_data0 = hexdump(caped_packet, dump=True)

    last_tree_entry = packet_data_tree.insert('', 'end', text='raw_data')
    for each_raw in raw_data0.split('\n'):
        packet_data_tree.insert(last_tree_entry, 'end', text=each_raw)
...
def track_tcp_ip_port():
    ...
    processed_packet = packet_list[local_packet_id]['processed_packet']
    ...
    listOf_entries_in_treeView = packet_tab_tree.get_children()
    for each in listOf_entries_in_treeView:
        ...
        if not check_is_save(packet_tab_tree.item(each)['values'],
                             ...
                             port=[track_src_port, track_dst_port],
                             flag=0, caped_packet=selected_packet):
...
def check_is_save(packet_info,
                  proto='', src_ip='', dst_ip='',
                  src_port='', dst_port='',
                  pid='', flag=-1, caped_packet='', ip=['', ''], port=['', '']):
    # print(packet_info[4])
    # [packet_id, packet_time, src_ip, dst_ip, proto,
    #     0            1        2       3       4
    # sport, dport, length, pid, info]
...
select_nif.pack(side=LEFT, after=track_ip_port_btn, padx=5, pady=10)
...
packet_tab_tree["columns"] = (
    "ID", "Time", "Src ip", "Dst ip", "Protocol", "Src port", "Dst port", "Length", "id", "Info")
